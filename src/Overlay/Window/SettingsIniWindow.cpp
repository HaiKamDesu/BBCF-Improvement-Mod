...
namespace {
	...
	static const SettingMetadata kSettingMetadata[] = {
		...
		{ "ShowHudWhenReplayPaused", "Keep the HUD up when a replay is paused", "Interface", "Pausing a replay normally makes the game hide its own interface, including its input history, which is exactly when you most want to read it. Turn this on to leave it on screen. Only affects Replay Theater, and the pause menu still works as it always did." },
...
#undef HOTKEY
	...
	static const SettingsPageInfo kSettingsPages[SPage_Count] = {
		...
		{ "Replays",      "The replay database and replay takeover.",                      "replay database upload takeover urt archive" },
