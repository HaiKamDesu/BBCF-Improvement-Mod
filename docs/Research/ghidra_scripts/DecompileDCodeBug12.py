# -*- coding: utf-8 -*-
# Ghidra headless Jython script, phase 12: AASTEAM_CUserManagedStorage vtable (the real transport behind D-code blob transfers; obj is 0x10 bytes, fields +8/+0xC init to -1, likely Steam async handles). Find the poll (+0x18), submit (+0x10), and the error state that wedges all fetches for the rest of the session.
...
# GAMESTEAM_CUserManagedStorageTransfer singleton (built by FUN_004B8F70, ctor
...
EXTRA_FUNCS = [
    0x004148f0,  # CUserManagedStorage ctor
    0x004b8b90,  # CUserManagedStorageTransfer base ctor
]
...
try:
    ...
    out.println("===== SYMBOLS matching UserManagedStorage =====")
    vtable_addrs = []
    st = currentProgram.getSymbolTable()
    it = st.getSymbolIterator()
    for sym in it:
        name = sym.getName(True)
        if "UserManagedStorage" in name:
