# -*- coding: utf-8 -*-
# Ghidra headless Jython script, phase 18: work-item factory + work manager class. The bbdc.dat paths (FUN_00422B00 dl / FUN_00423A50 share) stash {steamID, buffer, size} into work mgr DAT_00A5A050 then create a work item via FUN_004291D0(type 7=dl / 8=share) and poll mgr+4 for 7/8/9/0xB. Determine what the work item ACTUALLY does: Steam RemoteStorage UGC, or P2P keyed by steamID. Decompile the factory, the release fn, the mgr ctor, and dump the created classes.
...
FUNCTION_ADDRS = [
    0x004291d0,  # work item factory (type, ctx) -> work item
    0x00429390,  # work item release
    0x004276a0,  # work manager ctor (DAT_00A5A050)
    0x0041c820,  # frequent caller of the mgr getter (pump?)
    0x0046a970,  # another mgr getter caller
]
