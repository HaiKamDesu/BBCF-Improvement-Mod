# Ghidra headless Jython script: decompile the candidate generic message-key
# display functions found via string search (FUN_00407f20 = status/state text
# setter used across matchmaking flow, FUN_006983e0 = warning/error popup
# shower, FUN_00450c70 = D-Code specific status setter), and dump the FULL,
# untruncated caller list for each so we know every message key ever routed
# through them.
# Usage after import:
# analyzeHeadless <project_dir> BBCF -process BBCF.exe -noanalysis -scriptPath <script_dir> -postScript DecompileMessageFns.py <report_path>
...
FUNCTION_ADDRS = [
    ...
    0x006983e0,
    0x00450c70,
    0x00695830,  # also seen taking a message key as 2nd arg in several sites
    0x0070ecf0,  # also seen taking a message key
]
...
def dump_all_callers(out, addrs):
    out.println("===== FULL CALLER COUNTS (untruncated) =====")
    for addr_value in addrs:
        fn = get_fn(addr_value)
        if fn is None:
            continue
...
if len(args) > 0:
    ...
    report_file = File(File(currentProgram.getExecutablePath()).getParentFile(), "message_fns_decompile.txt")
...
try:
    ...
    dump_all_callers(out, FUNCTION_ADDRS)
finally:
    ifc.dispose()
    out.close()
