# -*- coding: utf-8 -*-
# Ghidra headless Jython script, phase 11: natural capture 2026-07-15 shows the
...
# GAMESTEAM_COnlineStorageTransfer singleton (built by FUN_004B8F70, ctor
...
EXTRA_FUNCS = [
    0x004b8b90,  # COnlineStorageTransfer base ctor
]
...
try:
    ...
    out.println("===== SYMBOLS matching OnlineStorage =====")
    vtable_addrs = []
    st = currentProgram.getSymbolTable()
    it = st.getSymbolIterator()
    for sym in it:
        name = sym.getName(True)
        if "OnlineStorage" in name:
